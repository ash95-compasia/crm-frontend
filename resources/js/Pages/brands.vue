        ...
        <div class="w-full p-6 lg:p-8">
                ...
                <div class="flex items-center justify-between mb-8">
                    <div class="flex items-center space-x-4">
                        <div>
                            <h1 class="text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white">Brand Management</h1>
                            <!-- Breadcrumb -->
                            <div class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 mt-2">
                                ...
                                <span class="text-gray-900 dark:text-gray-300 font-medium">Brands</span>
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="text-gray-600 dark:text-gray-400 text-sm">Total Brands</div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 dark:text-gray-400 text-sm">Total Brands</p>
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 dark:text-gray-400 text-sm">Active Brands</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ activeBrandsCount.toLocaleString() }}</p>
                            </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 dark:text-gray-400 text-sm">Inactive Brands</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ inactiveBrandsCount.toLocaleString() }}</p>
                            </div>
            ...
            <div class="fixed inset-0 overflow-y-auto">
                <div class="flex items-center justify-center min-h-full p-4">
                    <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full p-6">
                        ...
                        <!-- Form -->
                        <form @submit.prevent="modalMode === 'create' ? createBrand() : updateBrand()">
                            <div class="space-y-4">
                                <!-- Name -->
                                <div>
                            ...
                            <div class="flex justify-end space-x-4 mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                                ...
                                >
                                    ...
                                    {{ isSubmitting ? 'Saving...' : (modalMode === 'create' ? 'Create Brand' : 'Update Brand') }}
                                </button>
                            </div>
                        </form>
                    </div>
...
const activeBrandsCount = computed(() => {
    return tableData.value.filter(brand => brand.status === 1).length;
});

const inactiveBrandsCount = computed(() => {
    return tableData.value.filter(brand => brand.status === 0).length;
});
...
const createBrand = async () => {
    ...
    try {
        const response = await axios.post('/api/brands', formData.value, getAxiosConfig());
        ...
        if (error.response?.status === 422) {
            ...
            showNotification(error.response?.data?.message || 'Failed to create brand', 'error');
        }
    } finally {
...
const updateBrand = async () => {
    ...
    try {
        const response = await axios.put(`/api/brands/${formData.value.id}`, formData.value, getAxiosConfig());
        ...
        if (error.response?.status === 422) {
            ...
            showNotification(error.response?.data?.message || 'Failed to update brand', 'error');
        }
    } finally {
        isSubmitting.value = false;
    }
...
const deleteBrand = async (brand) => {
    ...
    try {
        const response = await axios.delete(`/api/brands/${brand.id}`, getAxiosConfig());
        ...
        showNotification(error.response?.data?.message || 'Failed to delete brand', 'error');
    }
};
...
const handleBulkAction = async (action, selectedIds) => {
    ...
    if (action === 'delete') {
        ...
        for (const id of selectedIds) {
            try {
                await axios.delete(`/api/brands/${id}`, getAxiosConfig());
            } catch (error) {
                console.error(`Failed to delete brand ${id}:`, error);
            }
        }

        showNotification(`${selectedIds.length} brand(s) deleted successfully!`, 'success');
        await fetchData();
        return;
    }
    ...
    try {
        const response = await axios.post('/api/brands/bulk-status', {
            ids: selectedIds,
            status: status
        }, getAxiosConfig());
        
        if (response.data.success) {
            showNotification(`${selectedIds.length} brand(s) updated successfully!`, 'success');
            await fetchData();
        } else {
            showNotification(response.data.message, 'error');
        }
    } catch (error) {
        showNotification(error.response?.data?.message || 'Failed to update brands', 'error');
    }
};
