        ...
        <div class="w-full p-6 lg:p-8">
                ...
                <div class="flex items-center justify-between mb-8">
                    <div class="flex items-center space-x-4">
                        <div>
                            <h1 class="text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white">Category Management</h1>
                            <!-- Breadcrumb -->
                            <div class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 mt-2">
                                ...
                                <span class="text-gray-900 dark:text-gray-300 font-medium">Categories</span>
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="text-right">                        <div class="text-gray-600 dark:text-gray-400 text-sm">Total Categories</div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 dark:text-gray-400 text-sm">Active Categories</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ activeCategoriesCount.toLocaleString() }}</p>
                            </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 dark:text-gray-400 text-sm">Inactive Categories</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ inactiveCategoriesCount.toLocaleString() }}</p>
                            </div>
            ...
            <div class="fixed inset-0 overflow-y-auto">
                <div class="flex items-center justify-center min-h-full p-4">
                    <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full p-6">
                        ...
                        <!-- Form -->
                        <form @submit.prevent="modalMode === 'create' ? createCategory() : updateCategory()">
                            <div class="space-y-4">
                                <!-- Name -->
                                <div>
...
const activeCategoriesCount = computed(() => {
    return tableData.value.filter(category => category.status === 1).length;
});

const inactiveCategoriesCount = computed(() => {
    return tableData.value.filter(category => category.status === 0).length;
});
...
const createCategory = async () => {
    ...
    try {
        const response = await axios.post('/api/categories', formData.value, getAxiosConfig());
...
const updateCategory = async () => {
    ...
    try {
        const response = await axios.put(`/api/categories/${formData.value.id}`, formData.value, getAxiosConfig());
...
const deleteCategory = async (category) => {
    ...
    try {
        const response = await axios.delete(`/api/categories/${category.id}`, getAxiosConfig());
...
const handleBulkAction = async (action, selectedIds) => {
    ...
    if (action === 'delete') {
        if (!confirm(`Are you sure you want to delete ${selectedIds.length} category(s)?`)) {
            return;
        }
        ...
        showNotification(`${selectedIds.length} category(s) deleted successfully!`, 'success');
        await fetchData();
        return;
    }
    ...
    try {
        const response = await axios.post('/api/categories/bulk-status', {
            ids: selectedIds,
            status: status
        }, getAxiosConfig());
        
        if (response.data.success) {
            showNotification(`${selectedIds.length} category(s) updated successfully!`, 'success');
            await fetchData();
        } else {
            showNotification(response.data.message, 'error');
        }
    } catch (error) {
        showNotification(error.response?.data?.message || 'Failed to update categories', 'error');
    }
};
